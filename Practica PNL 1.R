################# PrÃ¡ctico PLN 1Âª Parte #######################################

#Como en todo ejercicio de anÃ¡lisis de texto a travÃ©s de lenguaje natural, vamos a explorar el df con el 
#objetivo puesto en entender de quÃ© trata este corpus de texto. Por eso, en principio, no damos nunguna pista de 
#quÃ© contenidos tiene este corpus y partimos de la base de que hay que descubrirlo.
...
#1)Superficialmente, ¿De quÃ¨ trata este corpus? Utilizar funciones exploratorias
...
#Palabras más frecuentes en el corpus: feminista, feminismo, feministas, t.co, https, mujeres, maradona, mujer, muerte, mierda. Como aparecen expresiones no lexemáticas (t.co, https), busqué una forma de eliminarlas.
...
#2) Â¿En quÃ© otros idiomas fueron escritos estos tweets? Me quedo con aquellos tweets que solo han sido escritos en espaÃ±ol
freq(df$lang)
...
#3)Â¿CuÃ¡les son las 50 palabras mÃ¡s frecuentes para estos tweets?
dfpalabras <- df1 %>% unnest_tokens(word, text) %>% 
  anti_join(my_stopwords) %>%
  count(word, sort = TRUE)
palabrasmasfrecuentes <- dfpalabras %>% top_n(50)
#feminista, feministas, feminismo, mujeres, maradona, mujer, muerte, mierda, re, aborto, festejar, causa, pelotuda, ideologia, reforzar, hombres, derechos, movimiento, lucha, sos, vos, años, militancia, género, violencia, país, hora, chile, diego, silencio, frente, hablar, hombre, ley, vida, varones, gente, favor, bueno, quiero, violadores, acá, femicidios, motuda, estado, social, popular, partido, problema, izquierda. Entre las palabras hay algunos argentinismos frecuentes y derivados del voseo: re, sos, vos, acá, que quizá sean menos significativos como palabras coloquiales de uso común. 



#4)  Ahora, podemos hacernos una idea de lo que tratan estos tweets, pero podrÃ­amos ir un poco más allÃ¡ con otras herramientas. TambiÃ©n podrÃ­amos explorar n-grams. AquÃ­ podrÃ­amos explorar el nÃºmero de n-grams que quisiÃ©ramos, pero lo recomendable en PLN es explorar hasta 3-grams y no mÃ¡s ya que suele ser infructuoso. 
#Veamos entonces Â¿cuÃ¡les son los 10 bigramas y los 10 trigramas mÃ¡s comunes? Recordemos que aquÃ­ la estrategia para limpiar palabras vacÃ­as no es anti_join
...
#10 sandra pizarro víctima         1001

#5) Â¿QuÃ© nodos de significado existen y cuÃ¡les son son los nÃºcleos mÃ¡s importantes? Lo graficamos a travÃ©s de redes de n.gramas. Recordar que para graficar estas redes, los bigramas tienen que estar separados.
